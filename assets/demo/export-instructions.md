...
```bash
ffmpeg -i the-hive-skill-demo.mp4 -vf "fps=15,scale=1200:-1:flags=lanczos,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5" assets/demo/the-hive-skill-demo.gif
```
